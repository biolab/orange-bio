...
try:
    ...
    orngRegistry.addWidgetCategory("Orange Genomics", \
        os.path.join(distutils.sysconfig.get_python_lib(),"Genomics"), \
...
if len(sys.argv) == 0 or len(sys.argv) > 1 and "remove" not in sys.argv[1]:
    try:
        ...
        sys.path.insert(0, 'widgets') # temporary fix, until orngEnviron and orngRegistry are fixed, and automatically add paths to add-on's modules and widgets into sys.path
